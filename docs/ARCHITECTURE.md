...
- **JD sourcing** (`ai/jd_resolve.py::resolve_jd_texts()`): reads the job description from whatever's already in the application's Attachments tab (up to 3 most recent file-type Events' attachments, extracted via the same subprocess-timeout-wrapped `cv_extract.py::extract_text_from_file()` used for CV uploads) — there's no dedicated "upload JD" field, so if several files are attached, the model itself judges from context which one is the actual posting. Falls back to a live scrape of a LinkedIn `stellenanzeige_url` (cached on `Application.jd_link_text_cache`, invalidated in `update_application()` when the URL itself changes) only when no attachment yields text. An application with neither is left unscored (`match_score`/`success_probability` stay `null`) rather than the model guessing against nothing.
- **Batch endpoint:** `POST /api/applications/score-all` — background task iterating all of the account's applications, wired into the same `sync_common.py` progress/batch-result mechanism as the other global sync sources (`ai_scoring` key), with the same provider-based throttle (5s gemini/groq, 1s else) the removed traffic-light assessment used; stops the whole batch on `AINotConfigured`/`AIRateLimited`, continues past any other per-app error

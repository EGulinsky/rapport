...
- **Windows: a real Windows Installer package, no Python at all** (`installer/packaging/windows-wix/`) — built with the [WiX Toolset](https://wixtoolset.org/) rather than PyInstaller+NSIS, per explicit direction to use a proper installer framework instead of a custom console/GUI Python app. Two WiX v5 SDK-style projects (`WixToolset.Sdk` via NuGet, `dotnet build`, no separate CLI install):
  - **`RapportPackage/`** — the MSI: installs `docker-compose.yml` (version-resolved at build time) and `start-rapport.bat` into `Program Files\Rapport`, plus a "Start rapport" Start Menu shortcut. Fixed `UpgradeCode` + `MajorUpgrade` gives genuine version-aware upgrades (installing a newer Setup.exe replaces the old install) — something the previous NSIS script never had. Windows Installer itself provides the Add/Remove Programs entry and uninstaller natively.
  - **`RapportBundle/`** — a Burn bootstrapper (`bal:WixStandardBootstrapperApplication`) wrapping that MSI into the single `Rapport-Setup-<version>.exe` end users download, with a license/progress/finish wizard. The finish page's "Launch" checkbox runs `start-rapport.bat` via `LaunchTarget`, unelevated in the logged-in user's session — deliberately not a deferred custom action inside the elevated MSI install, since Docker Desktop's named pipe is only reliably reachable from an interactive user account.
  - **`RapportPackage/start-rapport.bat`** — the actual bootstrap logic as a plain batch script: check for Docker, download+silently install Docker Desktop if missing (`curl` + `install --quiet --accept-license`, handling exit code 3010 "restart required" distinctly), wait for the daemon, `docker compose pull && up -d`, poll `/health` via `curl`, open the browser — the same step sequence the old Python `docker_install/windows.py` had, just expressed as batch. Deliberately **no** Burn `ExePackage` prerequisite-chaining for Docker: Docker Desktop's installer is a vendor-hosted binary with no stable published hash, so pinning it as a chain payload (which requires a fixed size+hash) would break every future install the moment Docker updates it — batch-script logic sidesteps that fragility at the cost of the install happening after the wizard UI closes rather than mid-wizard.
  - **`build_windows_wix.ps1`** — resolves `docker-compose.yml` and generates `License.rtf` from the repo's actual `LICENSE` at build time (never checked in), then `dotnet build`s the Bundle project (pulls in the Package project via `ProjectReference`).
  - Verification caveat: WiX v5 is a cross-platform .NET tool and `dotnet build` runs on macOS too (catching real XML-authoring mistakes during development), but the toolset explicitly warns it "only supports Windows... all behavior after this point is undefined," and even a minimal textbook `<Directory Name="...">` element fails to compile under a non-Windows host (confirmed via isolated reproduction) — so the actual MSI/Burn compilation is only provable on `release.yml`'s `windows-latest` runner, the same verification bar the old NSIS build had.

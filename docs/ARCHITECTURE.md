...
- **Flow (`installer/main.py`):** check `docker info` → if Docker is missing, install it (`installer/docker_install/{macos,windows,linux}.py`, same `platform.system()`-dispatch pattern as `agent/service.py`) → resolve and write the bundled `compose_template.yml` (this build's stamped version substituted for `__VERSION__`, see `installer/version.py`) to a per-OS app-data dir → `docker compose pull && up -d` → poll `/health` (same 30-attempt/2s shape as `ci.yml`'s deploy job) → open `http://localhost:3000` in the default browser. On Windows, `main.py`'s `__main__` block dispatches (by `platform.system()`) to `installer/gui.py` instead of running this console flow directly — `main()` itself, and `tests/test_main.py`'s coverage of it, are untouched; `gui.py` is a parallel, self-contained Tkinter implementation of the same step sequence (not a refactor) so the console flow keeps working unchanged on macOS/Linux.
- **`installer/gui.py` (Windows only):** a real wizard window — status line, progress bar, scrollable log — instead of a bare console. Runs the bootstrap in a background thread and bridges progress to the Tkinter main thread via a `queue.Queue` drained on a `root.after()` timer (Tkinter isn't thread-safe); existing `print()` calls from `docker_install/windows.py` reach the log box unchanged via a small `sys.stdout`-redirecting `_QueueWriter`. On any failure the window stays open with a Retry button rather than risking a console that flashes closed; on success an "Open rapport" button re-opens the browser. `subprocess` calls from this module pass `creationflags=subprocess.CREATE_NO_WINDOW` since the packaged exe now runs with `console=False` and would otherwise flash a console per `docker`/`docker compose` invocation.
- **Docker install per OS** — all three are a fully scripted, silent install (not a "please install Docker yourself" page), still triggering the OS's own one-time admin/password prompt (unavoidable for privileged virtualization software): macOS mounts the official `.dmg` and runs Docker Desktop's documented MDM-style silent-install binary (`Docker.app/Contents/MacOS/install --accept-license`, the same mechanism tools like Jamf use); Windows runs the official installer with its documented `install --quiet --accept-license` flags, distinctly surfacing the "restart required" exit code (3010, e.g. when WSL2 is enabled for the first time) instead of treating it as a generic failure; Linux runs the official `get.docker.com` convenience script (Docker Engine, not Docker Desktop — no GUI needed for a locally self-hosted service) followed by a best-effort `usermod -aG docker` (a fresh install still needs `sudo docker` for the rest of that same run, since group membership only takes effect in a new login session — `docker_check.py`'s `docker_cmd_prefix()` picks plain vs. `sudo`-prefixed commands based on which one actually works).
...
- **Packaging:** same PyInstaller-onedir-per-OS pattern as the agent (`installer/packaging/installer-{mac,windows,linux}.spec` / `build_{dmg.sh,windows.ps1,linux.sh}`). macOS/Linux use `console=True` (a visible terminal showing live progress, since this is a one-shot flow the user watches rather than a hidden background service); Windows uses `console=False` since `gui.py`'s wizard window is the UI there instead. Each build script stamps the target version into `installer/version.py` immediately before invoking PyInstaller and reverts it afterward via a shell trap (even on failure), so a local build never leaves the working tree dirty.
- **Windows setup wizard:** unlike macOS (`.dmg`) and Linux (`.tar.gz`), the Windows onedir bundle is wrapped in a real graphical installer via NSIS (Nullsoft Scriptable Install System, free/open source under the zlib/libpng license) — `installer/packaging/installer-windows.nsi`, compiled by `build_windows.ps1` right after the PyInstaller step (requires `makensis` on `PATH`; `release.yml`'s `installer-windows` job installs it via `choco install nsis`). Produces `Rapport-Setup-<version>.exe`: a Modern-UI wizard (welcome → this repo's `LICENSE` → install-directory picker → progress → finish, with a "run now" checkbox), a Start Menu shortcut, and an uninstaller registered under `HKLM\...\Uninstall\RapportInstaller` (so it shows up in Add/Remove Programs) — replacing the previous bare `Rapport-Installer-<version>-windows.zip`.
...
**`release.yml`** is a second, deliberately decoupled workflow — it never runs on a push, only `on: release: types: [published]` (or manual `workflow_dispatch`), since the agent/installer packaged builds and GHCR images should only be rebuilt when a version is actually released, not on every commit. Seven jobs, all matrixed by platform where relevant: `publish-images` (backend/frontend/playwright-base → GHCR, see §3.8), `macos`/`windows`/`linux` (Rapport Agent installers, see §3.5), and `installer-macos`/`installer-windows`/`installer-linux` (Rapport Installer, see §3.8 — `installer-windows` also installs NSIS via `choco` to compile the setup wizard) — each resolves the release tag, builds via the matching `packaging/build_*` script, and `gh release upload`s the result to the same release.

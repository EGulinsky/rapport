...
**#230 investigation, part three: the application itself had no `datum_bewerbung`, so nothing was ever date-filtered — fixed with a shared floor applied uniformly to mail, calendar, and call-log sync, then revised the same day to key off the timeline instead of the application date.** `Application` has no `created_at`/timestamp column at all; `datum_bewerbung` is user-settable and can be left blank (e.g. a LinkedIn-scraped application with no discoverable applied date), in which case the original `_predates_bewerbung(datum, app)` compared against `None` and always returned `False` — no filtering happened, so #230's sync pulled in mail/calendar/call items spanning many months of unrelated history. `sync_common.py`'s `effective_bewerbung_floor(app)` closed that gap, and a same-day follow-up revision changed what it keys off: rather than `datum_bewerbung` (or a `letztes_update` fallback), the floor is now the **earliest dated `Event` already in the application's timeline** — `min(e.datum for e in app.events if e.datum)`. Reasoning: a recruiter call or an informal prep email can genuinely predate the day the formal application was submitted, and excluding it just because it's earlier than a user-entered application date was itself a (milder) false-negative version of the original bug. When an application has no dated events yet, the floor falls back to a loose 365-day lookback from today (`_LOOSE_FLOOR_FALLBACK_DAYS`) — `effective_bewerbung_floor()` always returns a concrete `date`, never `None`, so callers no longer need their own separate fallback. `earliest_bewerbung_date(db)` (the *global*, cross-application pre-filter used to cheaply skip messages in `_do_icloud_mail`'s bulk loop before per-app matching starts) was updated the same way, taking `min(MIN(Event.datum) across all events, today - 365 days)` so it can never be later — i.e. stricter — than any single application's own floor. This floor pair is the sole gate used across every timed-item sync path — `_sync_gcal_for_app`, `_sync_icloud_mail_for_app`, `_sync_icloud_cal_for_app`, and `_sync_calls_for_app` in `sync_targeted.py`, plus bulk `_do_icloud_calls` in `sync_icloud.py` (bulk `_do_gcal`/`_do_icloud_cal` and targeted `_sync_icloud_reminders_for_app` already routed through the shared `process_item()`, so they inherited the fix automatically). Calls sync in particular had *no* date filtering at all before the first pass — a call was matched purely by phone number/name, regardless of when it happened; the 365-day fallback is what now bounds a brand-new, event-less application's very first calls sync. A deliberate design choice from the same request: automated/ATS sender addresses are **not** blocked as a category — relevance is established by the date floor plus the existing company/role/domain matching, since a genuine HR-tool confirmation email is exactly the kind of item this sync exists to catch.

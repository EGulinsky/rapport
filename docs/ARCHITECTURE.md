...
- **rapportGPT** (`ai/chat.py`, `routers/chat.py`) — a tool-calling chat assistant replacing the earlier fixed per-application "AI assessment" feature (`Application.ai_color`/`ai_next_step`/`ai_reasoning`/`ai_assessed_at` columns still exist with historical values, but nothing writes to them anymore). One ongoing conversation thread per account (`ChatMessage`, tenant-scoped), persisted across reloads:
  ...
  - Five tools available to the model: `list_applications`, `get_application_detail` (full timeline, contacts, company), `get_company_detail`, `get_user_profile` (CV/LinkedIn profile text), and `add_assessment_feedback` (v4.7.16, the first *mutating* tool — see below) — each a plain tenant-scoped ORM query (or write), dispatched via `_execute_tool()` with per-call error isolation
...
- `match_score` — how well the job's requirements match the account's CV + LinkedIn profile text (both already cached on `User`). Graded deliberately critically (v4.7.16): the system prompt frames the model as a demanding hiring-manager persona in a buyer's market ("Bewerbermarkt zugunsten der Firma") who doesn't give the benefit of the doubt on ambiguous fits, plus an explicit instruction to dock points for every unmet/partially-met core requirement — a score above 80 is reserved for near-complete requirement coverage, not general seniority/soft-skill overlap.
- `success_probability` — a **second, separate** AI call fed the match score, the full timeline (`ai/timeline_text.py::build_timeline_text()`), the real ghosting signal (`apply_ghosting_overrides()`), and three additional richer signals added in v4.7.16 so the estimate isn't driven by raw activity/progress alone:
  - **Contact frequency + direction** (`ai/tasks.py::_build_activity_stats_block()`) — a code-computed summary (not left to the model to infer from free text) of total dated events, distinct linked contacts, and a received-vs-sent tally from `Event.mail_direction` — i.e. whether the company is reaching out proactively or the applicant is always the one initiating.
  - **Candidate count** (`Application.bewerberzahl`, see below) — included in the same block when set, as a point-in-time competitive-pressure signal.
  - **Historical comparable-stage outcomes** (`ai/historical_outcomes.py::compute_stage_outcomes()`) — among the account's *other* terminal applications (rejected/signed), how many reached *at least the same pipeline stage* this application currently sits at. A rejected application's stage-reached is `Application.pre_rejection_status` (already recorded — the stage right before rejection); rows predating that field, or with fewer than 2 comparable applications total, are excluded rather than guessed at. Rendered as e.g. "of 5 applications that also reached HR, 2 were signed and 3 rejected."
  - For terminal statuses (`rejected`/`signed`) `success_probability` is still set deterministically (0/100), skipping the second call entirely — the outcome is already known, so none of the above signals are computed for that case.
- **Persistent user feedback** (`ApplicationFeedback` model, v4.7.16) — an append-only log of free-text notes the user adds via rapportGPT's `add_assessment_feedback(application_id, feedback)` tool (e.g. "the role actually needs 10 years of Java, I have none"). Tenant-scoped like every other chat tool (the session's `with_loader_criteria` filter already restricts the lookup). After saving, the tool immediately re-runs `score_application()` best-effort (same `AINotConfigured`/`AIRateLimited`/generic-exception swallow contract as the sync's own scoring step) so the user sees updated numbers in the same chat reply. All stored feedback for an application is fed into **both** `compute_match_score()` and `compute_success_probability()` (`ai/tasks.py::_build_feedback_block()`) on every future recompute — a correction might target either assessment. Shown read-only (newest first) in `ApplicationModal.tsx`'s Overview tab, no edit/delete UI there — the log is chat-only by design.
...
**Candidate count** (`Application.bewerberzahl`, v4.7.16) — an optional "number of candidates" integer, editable directly in `ApplicationModal.tsx`'s Overview tab and, where possible, prefilled automatically from LinkedIn: `linkedin_job_description.py::load_job_description()`'s third structural heuristic (`_APPLICANT_COUNT_JS`, alongside the existing description/company-name heuristics — LinkedIn hashes all CSS class names, so this scans for a text node matching "X Bewerber"/"X applicants"/"X people clicked apply" rather than using a selector) feeds it into the existing "extract from LinkedIn URL" prefill flow (`POST /api/applications/extract-from-linkedin-url`). Deliberately a one-time prefill tied to that manual action, not part of the recurring/automatic job sync — LinkedIn's own "My Jobs" list-view tracker never exposes an applicant count in its card text (only the full job-posting page does), so re-scraping it on every sync would cost an extra full page load per application for no clear benefit over the prefill-once semantics.
...
| `bewerberzahl` | INTEGER NULL | Optional "number of candidates" — manually editable, and prefilled where possible from LinkedIn's own applicant-count text via the "extract from LinkedIn URL" flow (see §3.6) |

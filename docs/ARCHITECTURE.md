...
4. **Sync own connections list** (`_scrape_linkedin_connections()`/`_sync_contacts_from_linkedin()`, v4.7.11) — reuses `_get_linkedin_context()`'s existing-session-only Playwright context (no login attempt) to scrape `mynetwork/invite-connect/connections/`, following the same "click Next" pagination as the job-tracker scraper and the same card-parsing core (`_parse_people_anchors()`, shared with the people-search scraper) since both pages render identical card markup. Off by default (`SyncSettings.linkedin_contacts_enabled`, `DEFAULT 0`) — unlike Google Contacts' `DEFAULT 1` backfill of already-existing behavior, this is a genuinely new and riskier scrape (LinkedIn's own connections page, more sensitive to automated access than the already-scraped job-search pages), so nobody is auto-opted in. No first/last-name split is attempted on a connection's display name (same "don't guess" choice as the manual people-search import) — the parsed dict feeds the same `_sync_contacts_from_parsed()` pipeline iCloud/Google contacts already use (unconditional import, mention-based application linking), wired into `_sync_all_contacts()` (`sync_icloud.py`) as a third provider alongside iCloud/Google, gated on both the toggle and an existing `LinkedInSync.session_cookies`.

**Unified contacts sync + per-source progress (`_sync_all_contacts()`, `sync_icloud.py`):** the single orchestrator behind both the global "Sync all" button and the Contacts-tab's own sync button, running iCloud CardDAV, Google People API, and (v4.7.11) LinkedIn connections — each provider silently skipped if unconfigured/disabled, errors from one provider never blocking the others. Progress is tracked per source (`SyncProgress` keyed `icloud_contacts`/`google_contacts`/`linkedin_contacts` in `sync_common.py`), and the frontend (`ContactsView.tsx`, v4.7.11) renders each active source as its own row — live progress bar, the contact currently being processed (`current_item`), and running created/updated/skipped counts — instead of one merged bar with no per-source detail.

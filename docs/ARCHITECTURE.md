...
**#230 investigation, part three: the application itself had no `datum_bewerbung`, so nothing was ever date-filtered — fixed with a shared floor applied uniformly to mail, calendar, and call-log sync.** `Application` has no `created_at`/timestamp column at all; `datum_bewerbung` is user-settable and can be left blank (e.g. a LinkedIn-scraped application with no discoverable applied date), in which case the old `_predates_bewerbung(datum, app)` compared against `None` and always returned `False` — no filtering happened, so #230's sync pulled in mail/calendar/call items spanning many months of unrelated history. `sync_common.py` now exposes `effective_bewerbung_floor(app)`, returning `app.datum_bewerbung or app.letztes_update` (the latter defaults to the creation date whenever `datum_bewerbung` is left blank — see `create_application()`), and `_predates_bewerbung()` compares against that floor instead; both are `None` only in the genuinely rare case where neither field is set, in which case callers fall back to their own loose window (365 days) rather than filtering nothing. This single pair is now the sole gate used across every timed-item sync path — `_sync_gcal_for_app`, `_sync_icloud_mail_for_app`, `_sync_icloud_cal_for_app`, and `_sync_calls_for_app` in `sync_targeted.py`, plus bulk `_do_icloud_calls` in `sync_icloud.py` (bulk `_do_gcal`/`_do_icloud_cal` and targeted `_sync_icloud_reminders_for_app` already routed through the shared `process_item()`, so they inherited the fix automatically). Calls sync in particular had *no* date filtering at all before this — a call was matched purely by phone number/name, regardless of when it happened; a coincidental phone-number match could otherwise attribute an unrelated personal call to any application linked to that contact, however old. A deliberate design choice from the same request: automated/ATS sender addresses are **not** blocked as a category — relevance is established by the date floor plus the existing company/role/domain matching, since a genuine HR-tool confirmation email is exactly the kind of item this sync exists to catch.

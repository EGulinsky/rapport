...
flowchart LR
    ...
    I --> J["ios-deploy (self-hosted)<br/>build for device + devicectl install+launch<br/>on the paired CG iPad Air 7"]
...
| `ios-deploy` | push/PR to `main` (self-hosted, after `ios` passes) | Builds against the real device destination (`-allowProvisioningUpdates`, free-Apple-ID personal-team signing — `project.yml`'s `DEVELOPMENT_TEAM`), then `xcrun devicectl device install app` + `process launch` onto one specific paired iPad ("CG iPad Air 7") over its coredevice Wi-Fi tunnel. Not TestFlight/App Store — a personal-team install expires after ~7 days, which running on every push naturally refreshes. Only succeeds while that exact device is reachable from this Mac; unreachable is an environment condition, not a code regression |

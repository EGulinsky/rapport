...
- **Windows: a real Windows Installer package, no Python at all** (`installer/packaging/windows-wix/RapportPackage/`) — a single MSI built with the [WiX Toolset](https://wixtoolset.org/) rather than PyInstaller+NSIS, per explicit direction to use a proper installer framework instead of a custom console/GUI Python app, and deliberately not wrapped in an extra `.exe` bootstrapper (a WiX v4 Burn bundle was tried first and worked, but the simpler single-MSI form was preferred). A WiX v5 SDK-style project (`WixToolset.Sdk` via NuGet, `dotnet build`, no separate CLI install):
  - **`Product.wxs`** — installs `docker-compose.yml` (version-resolved at build time) and `start-rapport.bat` into `Program Files\Rapport`, plus a "Start rapport" Start Menu shortcut. Uses `WixUI_InstallDir` (`WixToolset.UI.wixext`) for the license/install-directory/progress/finish wizard, since the MSI is the whole installer here with no outer bootstrapper UI. Fixed `UpgradeCode` + `MajorUpgrade` gives genuine version-aware upgrades (installing a newer `Rapport-Setup.msi` replaces the old install) — something the previous NSIS script never had. Windows Installer itself provides the UAC elevation prompt, the Add/Remove Programs entry, and the uninstaller natively.
  ...
  - Verification caveat: WiX v5 is a cross-platform .NET tool and `dotnet build` runs on macOS too (catching real XML-authoring mistakes during development), but the toolset explicitly warns it "only supports Windows... all behavior after this point is undefined," and even a minimal textbook `<Directory Name="...">` element fails to compile under a non-Windows host (confirmed via isolated reproduction) — so the actual MSI compilation is only provable on `release.yml`'s `windows-latest` runner, the same verification bar the old NSIS build had. Three real, CI-diagnosed authoring errors were fixed this way before the first successful build: ICE80 component bitness (`Bitness="always64"`), the `Platform=x64` project property needed alongside it, and a required `Theme` attribute on the bootstrapper UI (the last one moot now that the Bundle project is gone, but the first two still apply to the MSI directly).

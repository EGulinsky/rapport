...
**Message import via CSV (no Playwright).** A live inbox scraper (`_scrape_messages()`) existed briefly but was removed (v4.5.5) — it only ever scrolled the conversation list once and reliably missed most conversations beyond whatever loaded initially, with no fix short of a different approach entirely. Replaced by an upload flow (Settings → LinkedIn → Messages): the user exports their own data from LinkedIn ("Get a copy of your data" → `messages.csv`) and uploads it via `POST /api/sync/linkedin/messages/import`. One `messages.csv` row is one message; import aggregates by `CONVERSATION ID` into one `LinkedInMessage` row per conversation (`participant_name_normalized` for matching, `last_message_date`/`preview`/`message_count`), re-upload updates existing conversations in place rather than duplicating (verified against a real 2907-row/759-conversation export, not just synthetic fixtures). The account owner's own name is auto-detected as the most frequent name across `FROM`/`TO` in the file, robust against it not exactly matching the account's stored name. `attach_linkedin_messages_for_contact(db, contact, user_id)` matches a contact to its conversations by normalized name (`sync_common._normalize_name()`) and creates one timeline `Event` per (application, conversation) pair — `source="linkedin_msg"`, `typ="mail"`, deduplicated the same way calls-sync events are, and gated by the same `_predates_bewerbung()` date floor as mail/calendar/call sync (a conversation dated before the application's earliest existing dated event, or an application with no dated events at all yet, creates no event — v4.6.2). `_normalize_name()` NFC-normalizes (precomposed vs. decomposed umlaut codepoints) and additionally transliterates German umlauts to their ASCII digraph form (ä→ae, ö→oe, ü→ue, ß→ss, applied after lowercasing) so a contact stored with a German umlaut in their name matches a CSV row spelling the same person with the ASCII digraph substitute instead, in either direction (v4.6.2) — this fingerprint function is shared with the existing mail-sync name+company fallback match, so both fixes apply there too. This runs at import time (across all contacts) and again from every contact-creation call site (`contacts.py`, `applications.py`, `sync_common.py`, `sync_targeted.py`, `sync_icloud.py` ×2, `sync_linkedin.py::import_people` — no shared choke point exists across these seven), so messages later attach retroactively the moment a matching contact is created by any means. Each event's `external_url` (v4.6.20) is set to the CSV's own `SENDER PROFILE URL`/`RECIPIENT PROFILE URLS` column for that message's participant — LinkedIn's own conversation-thread URLs can't be reliably reconstructed from just the export's `CONVERSATION ID`, so the timeline/contact modal instead open the participant's own LinkedIn profile, the closest available "open this on the web" target. `_backfill_linkedin_message_external_url()` (`database.py`) fills this in for events created before the column existed — a plain local join against the already-imported `linkedin_messages` table, no LinkedIn API access needed (unlike the Google Calendar equivalent), so it runs unconditionally on every startup rather than needing a manual endpoint.
...
- **`installer/`** — a downloadable, one-shot bootstrap app for the backend+frontend Docker stack, distinct from `agent/` (a persistent background service): a user downloads and runs it once, and rapport ends up running locally with no manual Docker install or `docker compose` terminal use. Unlike the agent it doesn't self-register or keep running — Docker's own `restart: unless-stopped` plus Docker Desktop's own "start at login" handle persistence across reboots once the containers are up once.
- **Flow (`installer/main.py`, macOS/Linux only):** check `docker info` → if Docker is missing, install it (`installer/docker_install/{macos,linux}.py`, same `platform.system()`-dispatch pattern as `agent/service.py`) → resolve and write the bundled `compose_template.yml` (this build's stamped version substituted for `__VERSION__`, see `installer/version.py`) to a per-OS app-data dir → `docker compose pull && up -d` → poll `/health` (same 30-attempt/2s shape as `ci.yml`'s deploy job) → open `http://localhost:3000` in the default browser. Windows has no Python installer at all — see below.
- **Docker install per OS (macOS/Linux)** — both are a fully scripted, silent install (not a "please install Docker yourself" page), still triggering the OS's own one-time admin/password prompt (unavoidable for privileged virtualization software): macOS mounts the official `.dmg` and runs Docker Desktop's documented MDM-style silent-install binary (`Docker.app/Contents/MacOS/install --accept-license`, the same mechanism tools like Jamf use); Linux runs the official `get.docker.com` convenience script (Docker Engine, not Docker Desktop — no GUI needed for a locally self-hosted service) followed by a best-effort `usermod -aG docker` (a fresh install still needs `sudo docker` for the rest of that same run, since group membership only takes effect in a new login session — `docker_check.py`'s `docker_cmd_prefix()` picks plain vs. `sudo`-prefixed commands based on which one actually works).
- **`compose_template.yml`** is deliberately slimmer than the repo-root `docker-compose.yml`: no Seq (a developer/ops convenience the backend already treats as optional and fails silently without, see `backend/app/logger.py`), and images are pulled prebuilt from GHCR instead of built locally (no Playwright/Chromium build step on the end user's machine).
...
- **Packaging (macOS/Linux):** PyInstaller-onedir pattern (`installer/packaging/installer-{mac,linux}.spec` / `build_{dmg.sh,linux.sh}`), `console=True` (a visible terminal showing live progress, since this is a one-shot flow the user watches rather than a hidden background service). Each build script stamps the target version into `installer/version.py` immediately before invoking PyInstaller and reverts it afterward via a shell trap (even on failure), so a local build never leaves the working tree dirty.
- **Windows: a real Windows Installer package, no Python at all** (`installer/packaging/windows-wix/RapportPackage/`) — a single MSI built with the [WiX Toolset](https://wixtoolset.org/) rather than PyInstaller+NSIS, per explicit direction to use a proper installer framework instead of a custom console/GUI Python app, and deliberately not wrapped in an extra `.exe` bootstrapper (a WiX v4 Burn bundle was tried first and worked, but the simpler single-MSI form was preferred). A WiX v5 SDK-style project (`WixToolset.Sdk` via NuGet, `dotnet build`, no separate CLI install):
  - **`Product.wxs`** — installs `docker-compose.yml` (version-resolved at build time) and `start-rapport.bat` into `Program Files\Rapport`, plus a "Start rapport" Start Menu shortcut. No custom WixUI dialog set: `WixUI_InstallDir` hit a known, unresolved WiX v5 bug (`wixtoolset/issues#7370`, "identifier inaccessible due to its protection level", closed "not planned") when referenced the standard way via `<UIRef>`, so this MSI relies on Windows Installer's own default progress UI instead of a custom license/directory-picker wizard. Fixed `UpgradeCode` + `MajorUpgrade` gives genuine version-aware upgrades (installing a newer `Rapport-Setup.msi` replaces the old install) — something the previous NSIS script never had. Windows Installer itself still provides the UAC elevation prompt, the Add/Remove Programs entry, and the uninstaller natively.
  - **Setup runs `start-rapport.bat` itself, automatically** — a `RunStartRapport` CustomAction (`WixQuietExec` from `WixToolset.Util.wixext`) is sequenced right after `InstallFiles`, so the whole Docker check/install/pull/start/open-browser sequence happens as part of Setup itself, no manual post-install step (an explicit revision from an earlier "run it from the Start Menu yourself" design, per direct feedback that a real installer should do everything itself). The Start Menu shortcut remains for re-running later. Because `WixQuietExec` runs with no visible console, `start-rapport.bat` was rewritten to never `pause` on error (an interactive-only affordance that would otherwise hang Setup indefinitely) — it just exits non-zero.
  - **`start-rapport.bat`** — check for Docker, download+silently install Docker Desktop if missing (`curl` + `install --quiet --accept-license`, handling exit code 3010 "restart required" distinctly), wait for the daemon, `docker compose pull && up -d`, poll `/health` via `curl`, open the browser — the same step sequence the old Python `docker_install/windows.py` had, just expressed as batch (no installer technology speaks Docker CLI natively, so shelling out is unavoidable either way).
  - UX tradeoff: Windows Installer's default progress bar has no visibility into `start-rapport.bat`'s own internal steps, so a fresh-Docker-install run can make Setup appear to sit still for several minutes while that happens inside the single CustomAction.
  - Verification gap specific to this behavior: `release.yml`'s `windows-latest` runner has no Docker Desktop installed, so it can only prove the MSI compiles and the CustomAction is wired correctly — the actual Docker-install-and-compose-up behavior, now running automatically during Setup, still needs a real Windows machine to exercise end-to-end.
  - **`build_windows_wix.ps1`** — resolves `docker-compose.yml` and generates `License.rtf` from the repo's actual `LICENSE` at build time (never checked in), then `dotnet build`s the MSI project directly.
  - Verification caveat: WiX v5 is a cross-platform .NET tool and `dotnet build` runs on macOS too (catching real XML-authoring mistakes during development), but the toolset explicitly warns it "only supports Windows... all behavior after this point is undefined," and even a minimal textbook `<Directory Name="...">` element fails to compile under a non-Windows host (confirmed via isolated reproduction) — so the actual MSI compilation is only provable on `release.yml`'s `windows-latest` runner, the same verification bar the old NSIS build had. Several real, CI-diagnosed authoring errors were fixed this way before the first successful build: ICE80 component bitness (`Bitness="always64"`), the `Platform=x64` project property needed alongside it, a required `Theme` attribute on the bootstrapper UI (from a Burn-bootstrapper version tried first and abandoned in favor of a plain MSI), and the `WixUI_InstallDir` protection-level bug noted above (also abandoned rather than worked around).
...
flowchart LR
    ...
    I --> J["ios-deploy (self-hosted)<br/>build for device + devicectl install (no auto-launch)<br/>on the paired CG iPad Air 7"]
...
| `ios-deploy` | push/PR to `main` (self-hosted, after `ios` passes) | Builds against the real device destination (`-allowProvisioningUpdates`, free-Apple-ID personal-team signing — `project.yml`'s `DEVELOPMENT_TEAM`), then `xcrun devicectl device install app` onto one specific paired iPad ("CG iPad Air 7") over its coredevice Wi-Fi tunnel. Deliberately install-only, no `process launch` — the app shouldn't force itself to the foreground on the device on every single push; open it by hand to pick up the new build. Not TestFlight/App Store — a personal-team install expires after ~7 days, which running on every push naturally refreshes. Only succeeds while that exact device is reachable from this Mac; unreachable is an environment condition, not a code regression |

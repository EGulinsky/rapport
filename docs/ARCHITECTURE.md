...
- **Packaging:** same PyInstaller-onedir-per-OS pattern as the agent (`installer/packaging/installer-{mac,windows,linux}.spec` / `build_{dmg.sh,windows.ps1,linux.sh}`), but `console=True` (a visible terminal showing live progress, since this is a one-shot flow the user watches rather than a hidden background service) and no tray-icon/GUI dependencies at all. Each build script stamps the target version into `installer/version.py` immediately before invoking PyInstaller and reverts it afterward via a shell trap (even on failure), so a local build never leaves the working tree dirty.

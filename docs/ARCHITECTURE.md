...
- **Windows: a real Windows Installer package, no Python at all** (`installer/packaging/windows-wix/RapportPackage/`) — a single MSI built with the [WiX Toolset](https://wixtoolset.org/) rather than PyInstaller+NSIS, per explicit direction to use a proper installer framework instead of a custom console/GUI Python app, and deliberately not wrapped in an extra `.exe` bootstrapper (a WiX v4 Burn bundle was tried first and worked, but the simpler single-MSI form was preferred). A WiX v5 SDK-style project (`WixToolset.Sdk` via NuGet, `dotnet build`, no separate CLI install):
  ...
  - **`start-rapport.bat`** — the actual bootstrap logic as a plain batch script, run manually from the "Start rapport" Start Menu shortcut after installing (no auto-launch-after-setup here, unlike the Burn-bundle version tried first — one extra click, no bootstrapper wrapper): check for Docker, download+silently install Docker Desktop if missing (`curl` + `install --quiet --accept-license`, handling exit code 3010 "restart required" distinctly), wait for the daemon, `docker compose pull && up -d`, poll `/health` via `curl`, open the browser — the same step sequence the old Python `docker_install/windows.py` had, just expressed as batch.

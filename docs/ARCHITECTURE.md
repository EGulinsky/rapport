...
| `GET` | `/api/contacts/{id}/events` | Calls, mails, and LinkedIn messages connected to this contact, newest-first (ContactModal.tsx tabs) — see §3.5 for the matching approach |
...
- **Attributing timeline events back to a specific contact** — none of calls/mails/LinkedIn messages has a direct FK to `Contact`, so `GET /api/contacts/{id}/events` (ContactModal.tsx's Calls/Mails/Messages tabs) matches each by whatever signal already exists on the `Event` row: calls and LinkedIn messages embed the contact's display name in `Event.titel` at creation time (`_delete_call_events_for_contact()` in `applications.py`, `attach_linkedin_messages_for_contact()` in `sync_linkedin.py`); mails match by sender address in `Event.autor`. `Event.autor` was silently unpopulated for every mail synced between the v3.14.6 "remove AI from sync" refactor (2026-06-29) and this fix — the AI-based classifier used to set it, and the deterministic replacement (`_save_deterministic_event()` in `sync_common.py`) never carried that line over. Restored going forward; mail events synced during that window have no recoverable sender and won't appear in the Mails tab.
...
**`release.yml`** is a second, deliberately decoupled workflow — it never runs on a push, only `on: release: types: [published]` (or manual `workflow_dispatch`), since the agent/installer packaged builds and GHCR images should only be rebuilt when a version is actually released, not on every commit. Seven jobs, all matrixed by platform where relevant: `publish-images` (backend/frontend/playwright-base → GHCR, see §3.8), `macos`/`windows`/`linux` (Rapport Agent installers, see §3.5), and `installer-macos`/`installer-windows`/`installer-linux` (Rapport Installer, see §3.8) — each resolves the release tag, builds via the matching `packaging/build_*` script, and `gh release upload`s the result to the same release.

...
**Gesamt: 53 % Line-Coverage über `app/` (8978 Statements, 4202 ungetestet, Stand 2026-07-06).** Der Durchschnitt verschleiert eine sehr ungleiche Verteilung, die dem Rollout-Plan (Abschnitt 11) entspricht — hoch dort, wo Phase 1–3 bewusst zuerst angesetzt haben, niedrig dort, wo Phase 4 noch aussteht:
...
| `sync_targeted.py` (1258 Zeilen, zweitgrößte Datei) | **57 %** (↑ von 5 %) | Logik-Helfer, API-Endpunkte, Domain-/Text-Filter für alle sechs Quellen (Gmail, Calendar, iCloud Mail/Calendar/Erinnerungen/Kontakte) sowie die agentenbasierten Quellen (Notizen, Anrufe) abgedeckt. Nur noch iCloud Notes offen |
| `sync_linkedin.py` (**37 %**), `review.py`, `main.py`, `analytics.py` | 24–37 % | Größtenteils ungetestet |
| `sync_icloud.py` (1275 Zeilen, größte Backend-Datei) | **45 %** (↑ von 23 %) | Mail (IMAP), Calendar + Reminders (CalDAV, inkl. Änderungserkennung/verwaiste Termine) und jetzt Contacts (CardDAV, inkl. Regressionstests für zwei live verifizierte Massenimport-Bugs) abgedeckt. Notes (pyicloud-API) offen |
...
| **4** 🔶 | Mocking-Infrastruktur für Gmail/iCloud/LinkedIn/AI + L3-Integrationstests | KI-Provider erledigt (`fake_ai_provider`, 16 Tests — inkl. vollständigem Fehler-Mapping: AINotConfigured, AuthenticationError, JSON-Modus nicht unterstützt, Modell nicht gefunden, generischer BadRequest). Google Calendar erledigt (`fake_google_calendar`, 7 Tests — inkl. "nicht verbunden"-Fällen). Gmail erledigt (`fake_gmail`, 9 Tests — inkl. zweiphasiger Batch-Abholung Metadata/Volltext, Pagination, teilweisem Batch-Fehler, "nicht verbunden"-Fällen). Google-Token-Refresh-Fehlerpfad (`_refresh_if_needed()`) separat mit 3 Component-Tests abgesichert (invalid_grant/revoked löschen Tokens, sonstige Fehler werden unverändert durchgereicht). LinkedIn-Statuslogik erledigt: `_process_linkedin_job()` aus der `_async_sync()`-Closure extrahiert (kein Playwright-Mock nötig, reine DB-Logik) und mit 9 Tests abgesichert — dabei einen echten Live-Bug gefunden und gefixt (neue Bewerbungen erzeugten für jeden Status einen No-op-Review-Eintrag "X → X" statt nur bei "rejected"; 8 solche Altlast-Einträge in der Produktiv-Review-Queue bestätigt). Zusätzlich der MergeAlias-Fallback in `_find_or_create_application()` mit 5 Component-Tests abgesichert (Alias über Job-ID/Firma+Rolle, Job-ID-Backfill, gelöschte kanonische Bewerbung, falscher entity_type). Eine gezielte Lücken-Analyse der bis dahin fehlenden Fehlerfälle (siehe Abschnitt 3) hat diese 5 Blindflecken identifiziert; alle sind jetzt geschlossen (18 neue Tests). Danach `sync_targeted.py` (gezielter Einzelbewerbungs-Sync, zweitgrößte Backend-Datei, war bei 5 % Coverage) mit 72 neuen Tests von 5 % auf 38 % gehoben: alle reinen Logik-Helfer (Suchbegriffe, Domain-Ableitung aus Firmenprofil/HH-Zielfirma/Kontakt-E-Mails, Text-Matching), die API-Endpunkte (Reset, Kandidatenliste Pool 1+2, manuelle Zuweisung inkl. Konflikterkennung), die domain-basierte Filter-Logik von Gmail/Calendar (eigene, schlankere Gmail-Fake nötig, da hier — anders als beim globalen Sync — direkt statt per Batch abgeholt wird) sowie die agentenbasierten Quellen (iCloud-Notizen, Anrufe, gemockt über `httpx.AsyncClient.get` statt echtem IMAP/CalDAV). Dabei einen echten Bug gefunden und gefixt: die direkt erzeugten Kalender-Events (Google + iCloud) in `sync_targeted.py` setzten `external_id` nicht (im Unterschied zum globalen Sync), wodurch `list_candidates()`/`manual_assign()` sie nicht zuverlässig wiederfinden konnten. Danach iCloud-Mocking begonnen: IMAP-Fake (`fake_icloud_imap`) für iCloud Mail (global + `sync_targeted.py`, 10 Tests) und CalDAV-Fake (`fake_caldav`, Events über echtes `vobject.readOne()`-Parsing statt Hand-Stubs) für Calendar + Reminders (global + `sync_targeted.py`, 21 Tests) gebaut. Dabei einen zweiten, unabhängigen Bug in drei Funktionen gefunden und gefixt: `_do_icloud_cal()`/`_do_icloud_reminders()` (`sync_icloud.py`) und `_sync_icloud_reminders_for_app()` (`sync_targeted.py`) wandten `str()` direkt auf vobject-ContentLine-Objekte an statt auf `.value`, wodurch Kalender-/Erinnerungstitel als `<SUMMARY{}Text>` statt Klartext gespeichert wurden — als `vobj_str()` in `sync_common.py` zentralisiert und gefixt. `sync_icloud.py` damit von 23 % auf 43 %, `sync_targeted.py` von 38 % auf 53 % Coverage. Anschließend iCloud Contacts (CardDAV) ergänzt: `fetch_all_vcards()` (rohes XML-Wire-Protokoll ohne SDK) direkt als Mocking-Grenze gewählt, 14 Tests für `_sync_contacts_http()` (global) und `_sync_contacts_for_app()` (gezielt) inkl. `_parse_vcard()`-Unit-Tests über echte per `vobject` serialisierte vCards. Regressionstests für zwei live verifizierte Massenimport-Bugs ergänzt (verwaistes CompanyProfile ohne Bewerbungsanbindung bzw. ohne Domain-Match darf nicht importieren). `sync_icloud.py` damit auf 45 %, `sync_targeted.py` auf 57 % Coverage. Nur noch iCloud Notes (pyicloud-API) sowie LinkedIn-Playwright-Fixture-Replay (Nightly-Tier, Phase 6) offen |

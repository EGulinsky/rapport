...
>
> **Current scale (2026-07-16):** 1386 backend tests (415 unit / 251 component / 526 api / 194 integration) + 93 frontend tests (up from 11 — the bulk of the growth is the i18n project: per-component language-switch tests plus the `locales.test.ts` key-parity/interpolation-placeholder suite) + 133 agent tests (up from 64 — the portability effort's factory/service/Windows/Linux-provider coverage, see section 13), run in CI on a `[ubuntu-latest, windows-latest, macos-latest]` matrix. PR-gate coverage (`unit or component or api`) 75% of `app/` (10210 statements — grew from 9833 with the addition of `error_keys.py`/`i18n_strings.py`/`cv_extract.py`, the AI-assessment CV/LinkedIn-profile work, the post-create-sync orchestration, and the unified Gmail/iCloud Mail matching below); including integration tests, 87%. The growth since 2026-07-14 (27 new tests) is: 21 tests from the CV-caching-fix and post-create-sync work (see prior entries in this section's history — unchanged from before); and 6 new **integration** tests for unifying Gmail/iCloud Mail matching and adding company-name/role search (`sync_common.find_matching_apps()`, `build_firm_index()`'s new role indexing — see [ARCHITECTURE.md §5.1](ARCHITECTURE.md#51-targeted-sync-per-application)): a mail matched purely by company name or role text with no known contact (both sources), and a phase-2 full-body re-check surfacing a company mention the subject-only phase-1 pass couldn't see (Gmail). PR-gate coverage dipped slightly (76%→75%) since these new call sites are only exercised by the integration-marked tests, not unit/component/api. See section 10 for the 2026-07-11 per-module breakdown (still broadly representative — subsequent work added test files rather than shifting existing router coverage meaningfully).

...
**Gesamt: 52 % Line-Coverage über `app/` (8978 Statements, 4278 ungetestet, Stand 2026-07-06).** Der Durchschnitt verschleiert eine sehr ungleiche Verteilung, die dem Rollout-Plan (Abschnitt 11) entspricht — hoch dort, wo Phase 1–3 bewusst zuerst angesetzt haben, niedrig dort, wo Phase 4 noch aussteht:
...
| `sync_targeted.py` (1258 Zeilen, zweitgrößte Datei) | **53 %** (↑ von 5 %) | Logik-Helfer, API-Endpunkte, Domain-Filter (Gmail/Calendar/iCloud Mail/iCloud Calendar), agentenbasierte Quellen (Notizen, Anrufe) und iCloud-Erinnerungen abgedeckt. iCloud Contacts (CardDAV) innerhalb dieser Datei bleibt offen |
| `sync_linkedin.py` (**37 %**), `review.py`, `main.py`, `analytics.py` | 24–37 % | Größtenteils ungetestet |
| `sync_icloud.py` (1275 Zeilen, größte Backend-Datei) | **43 %** (↑ von 23 %) | Mail (IMAP), Calendar + Reminders (CalDAV, inkl. Änderungserkennung/verwaiste Termine) jetzt abgedeckt. Contacts (CardDAV) und Notes (pyicloud-API) offen |

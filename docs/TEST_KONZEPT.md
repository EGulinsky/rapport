...
>
> **Current scale (2026-07-16):** 1380 backend tests (415 unit / 251 component / 526 api / 188 integration) + 93 frontend tests (up from 11 — the bulk of the growth is the i18n project: per-component language-switch tests plus the `locales.test.ts` key-parity/interpolation-placeholder suite) + 133 agent tests (up from 64 — the portability effort's factory/service/Windows/Linux-provider coverage, see section 13), run in CI on a `[ubuntu-latest, windows-latest, macos-latest]` matrix. PR-gate coverage (`unit or component or api`) 76% of `app/` (10178 statements — grew from 9833 with the addition of `error_keys.py`/`i18n_strings.py`/`cv_extract.py`, the AI-assessment CV/LinkedIn-profile work, and the post-create-sync orchestration below); including integration tests, 87%. The growth since 2026-07-14 (21 new tests) is: 4 unit tests for the CV-extraction-caching migration's backfill + 2 API tests for the upload/delete wiring (`User.cv_extracted_text`, replacing per-assessment extraction — see [ARCHITECTURE.md §3.6](ARCHITECTURE.md#36-ai-classification-assessment-litellm)); 1 unit test proving CV extraction's new subprocess timeout actually kills a hung parse instead of blocking (see the "Event-loop safety" note below — this was a real production incident, not a hypothetical); and 14 tests (5 unit + 7 component + 2 API) for the new post-create-sync feature (`sync_targeted._do_post_create_sync()`, `sync_linkedin.run_individual_sync_if_idle()`/`_sync_newly_created_apps()` — see [ARCHITECTURE.md §5.2](ARCHITECTURE.md#52-linkedin-import-of-a-job-posting)). See section 10 for the 2026-07-11 per-module breakdown (still broadly representative — subsequent work added test files rather than shifting existing router coverage meaningfully).

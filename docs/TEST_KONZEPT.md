...
>
> **Current scale (2026-07-16):** 1388 backend tests (415 unit / 251 component / 526 api / 196 integration) + 93 frontend tests (up from 11 — the bulk of the growth is the i18n project: per-component language-switch tests plus the `locales.test.ts` key-parity/interpolation-placeholder suite) + 133 agent tests (up from 64 — the portability effort's factory/service/Windows/Linux-provider coverage, see section 13), run in CI on a `[ubuntu-latest, windows-latest, macos-latest]` matrix. PR-gate coverage (`unit or component or api`) 75% of `app/` (10221 statements — grew from 9833 with the addition of `error_keys.py`/`i18n_strings.py`/`cv_extract.py`, the AI-assessment CV/LinkedIn-profile work, the post-create-sync orchestration, and the unified Gmail/iCloud Mail matching below); including integration tests, 87%. The growth since 2026-07-14 (33 new tests) is: 21 tests from the CV-caching-fix and post-create-sync work; 6 integration tests for unifying Gmail/iCloud Mail matching and adding company-name/role search (`sync_common.find_matching_apps()`, `build_firm_index()`'s new role indexing — see [ARCHITECTURE.md §5.1](ARCHITECTURE.md#51-targeted-sync-per-application)); and 6 more (net) fixing the false-positive flood that surfaced in production the same day (`_search_terms()`'s role-as-whole-phrase change, `_GENERIC_ROLE_TERMS`, the targeted-sync re-verification and circuit-breaker — see ARCHITECTURE.md §5.1's "#230 incident" paragraph) — 3 new unit tests (`_search_terms` role handling), 3 new integration tests (circuit breaker, re-verification), and 3 old `_role_query_words` unit tests removed along with the function itself. See section 10 for the 2026-07-11 per-module breakdown (still broadly representative — subsequent work added test files rather than shifting existing router coverage meaningfully).

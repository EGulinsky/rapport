...
**Aktueller Stand (454 Tests insgesamt, Stand 2026-07-06):** `backend/tests/` (351 Tests, Marker `unit`/`component`/`api`), `agent/tests/` (51 Tests) und `frontend/src/**/*.test.tsx` (11 Tests) laufen bei jedem Push als Pflicht-Gate (siehe `ci.yml`). Zusätzlich laufen bei Push auf `main` 41 L3-Integrationstests (`pytest -m integration`: KI-Provider-Flow über `fake_ai_provider` inkl. vollständigem Fehler-Mapping, Google-Calendar-Sync über `fake_google_calendar`, Gmail-Sync inkl. zweiphasiger Batch-Abholung über `fake_gmail`, jeweils inkl. "nicht verbunden"-Fällen, sowie die domain-basierte Matching-Logik des gezielten Einzelbewerbungs-Syncs in `sync_targeted.py`). Gezielte Fehlerfall-Lücken-Analyse (siehe Abschnitt 3) hat zusätzlich den Google-Token-Refresh-Fehlerpfad und den LinkedIn-MergeAlias-Fallback als Blindflecken identifiziert und geschlossen. Bei den `sync_targeted.py`-Tests einen echten Bug gefunden und gefixt: die dort direkt erzeugten Kalender-Events (Google + iCloud) setzten `external_id` nicht, wodurch `list_candidates()`/`manual_assign()` sie nicht zuverlässig wiederfinden konnten. iCloud (IMAP/CalDAV/CardDAV) für die eigentlichen Sync-Quellen, LinkedIn-Playwright-Fixture-Replay, L4 E2E und der Nightly-Job aus diesem Konzept sind noch nicht umgesetzt — bis dahin werden diese Sync-/Scraping-Regressionen weiterhin nur durch manuelles Testen nach dem Deploy gefunden. Gemessene Zeilenabdeckung und wo die größten Lücken liegen: siehe [Abschnitt 10](#10-abdeckungsziele-vorschlag-kein-dogma).
...
**Gesamt: 47 % Line-Coverage über `app/` (8977 Statements, 4760 ungetestet, Stand 2026-07-06).** Der Durchschnitt verschleiert eine sehr ungleiche Verteilung, die dem Rollout-Plan (Abschnitt 11) entspricht — hoch dort, wo Phase 1–3 bewusst zuerst angesetzt haben, niedrig dort, wo Phase 4 noch aussteht:
...
| `sync_targeted.py` (1262 Zeilen, zweitgrößte Datei) | **38 %** (↑ von 5 %) | Alle reinen Logik-Helfer (Suchbegriffe, Domain-Ableitung, Text-Matching), die API-Endpunkte (Reset/Kandidaten/manuelle Zuweisung) sowie die Domain-Filter-Logik von Gmail/Calendar und die agentenbasierten Quellen (Notizen, Anrufe) abgedeckt. iCloud-Quellen (IMAP/CalDAV/CardDAV) innerhalb dieser Datei bleiben offen — siehe unten |
| `sync_linkedin.py` (**37 %**, ↑ von 35 % durch MergeAlias-Fallback-Tests), `sync_common.py`, `review.py`, `main.py`, `analytics.py` | 19–37 % | Größtenteils ungetestet |

...
>
> **Current scale (2026-07-16):** 1403 backend tests (426 unit / 252 component / 526 api / 199 integration) + 93 frontend tests (up from 11 — the bulk of the growth is the i18n project: per-component language-switch tests plus the `locales.test.ts` key-parity/interpolation-placeholder suite) + 133 agent tests (up from 64 — the portability effort's factory/service/Windows/Linux-provider coverage, see section 13), run in CI on a `[ubuntu-latest, windows-latest, macos-latest]` matrix. PR-gate coverage (`unit or component or api`) 75% of `app/` (10230 statements — grew from 9833 with the addition of `error_keys.py`/`i18n_strings.py`/`cv_extract.py`, the AI-assessment CV/LinkedIn-profile work, the post-create-sync orchestration, and the unified Gmail/iCloud Mail matching below); including integration tests, 87%. The growth since 2026-07-14 (48 new tests) is: 21 tests from the CV-caching-fix and post-create-sync work; 6 integration tests for unifying Gmail/iCloud Mail matching and adding company-name/role search; 6 (net) fixing the mail false-positive flood (whole-phrase role terms, `_GENERIC_ROLE_TERMS`, targeted-sync re-verification and circuit-breaker); 3 more fixing that same incident's cascade into calendar events and call-log entries (`_company_domains_for_app()`'s new `contacts` override parameter, `_do_sync()`'s domain-list snapshot via `app_dict["_domain_snapshot"]`); and 12 more (9 unit + 3 integration/component) for the follow-up date-floor fix — a shared `effective_bewerbung_floor()`/`_predates_bewerbung()` pair in `sync_common.py` now uniformly gates mail, calendar, and call-log sync (bulk and targeted) so nothing older than an application's applied date (or, absent that, its creation date) is ever considered a match — see [ARCHITECTURE.md §5.1](ARCHITECTURE.md#51-targeted-sync-per-application)'s "#230 incident" paragraphs for the full story. See section 10 for the 2026-07-11 per-module breakdown (still broadly representative — subsequent work added test files rather than shifting existing router coverage meaningfully).

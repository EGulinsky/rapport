...
>
> **Current scale (2026-07-14):** 1359 backend tests (405 unit / 244 component / 522 api / 188 integration) + 93 frontend tests (up from 11 — the bulk of the growth is the i18n project: per-component language-switch tests plus the `locales.test.ts` key-parity/interpolation-placeholder suite) + 133 agent tests (up from 64 — the portability effort's factory/service/Windows/Linux-provider coverage, see section 13), run in CI on a `[ubuntu-latest, windows-latest, macos-latest]` matrix. PR-gate coverage (`unit or component or api`) 75% of `app/` (10097 statements — grew from 9833 with the addition of `error_keys.py`/`i18n_strings.py`/`cv_extract.py` and the AI-assessment CV/LinkedIn-profile work); including integration tests, 87%. The growth since 2026-07-13 is the AI-assessment CV/LinkedIn-profile feature (29 new tests: extraction, LinkedIn profile scraping, prompt wiring, migration, API) plus a regression test proving the iCloud sync's event-loop-blocking fix actually works — see [ARCHITECTURE.md §3.6](ARCHITECTURE.md#36-ai-classification-assessment-litellm) and the "Event-loop safety" note below. See section 10 for the 2026-07-11 per-module breakdown (still broadly representative — subsequent work added test files rather than shifting existing router coverage meaningfully).

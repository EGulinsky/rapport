...
>
> **Current scale (2026-08-03):** 1975 backend tests (669 unit / 345 component / 683 api / 278 integration) + 195 frontend tests + 163 agent tests, run in CI on a `[ubuntu-latest, windows-latest, macos-latest]` matrix. The location/address autocomplete was migrated from a dual Google Places (paid, POIs/addresses) + Nominatim (free, cities-only) setup to Photon (komoot) — a single free, keyless, OpenStreetMap-based provider that returns cities, POIs, and full addresses in one call — and the paid Google Maps option (`MapsSettings` model, Settings → Maps panel, `/api/settings/maps*` endpoints) was removed entirely, since it was never actually used. `tests/component/test_geo.py` was reclassified to `tests/unit/test_geo.py` (no DB access needed anymore, the API-key DB lookup it used to require is gone); `tests/api/test_settings_maps_api.py` and `test_settings_api.py::TestMapsSettings` were deleted along with the feature. **v4.7.33 same-day follow-up:** the Photon migration shipped completely broken — verified live against the real API from inside the deployed backend container, Photon's nginx returns a bare 403 Forbidden for httpx's default `python-httpx/x.y.z` User-Agent, so every autocomplete/geocode/reverse-geocode call silently came back empty (each is wrapped in a best-effort `except (httpx.HTTPError, ValueError): return None`/`[]`, by design, so nothing crashed or logged an error — it just quietly did nothing). Fixed by setting a real identifying `User-Agent` header on all four `httpx.AsyncClient` instances in `geo.py` (Photon search/reverse/geocode plus OSRM routing, for consistency). Regression tests added: an assertion on `self.headers["user-agent"]` in the existing `fake_get` mocks across `test_geo.py`/`test_geo_routing.py`, so a future refactor that drops the header again fails a test instead of silently regressing in production the same way. PR-gate coverage (`unit or component or api`) 77% of `app/`; including integration tests, 89%. The backend job's CI timeout was cut from 25 to 10 minutes and `pytest-timeout` added (60s per-test cap, `signal` method) after a recurring, locally-unreproducible hang on GitHub's hosted runner — a stuck test now fails fast with a real stack trace instead of silently burning the whole job. See section 10 for the 2026-07-11 per-module breakdown (still broadly representative — subsequent work added test files rather than shifting existing router coverage meaningfully). New test files from the AI match-score/success-probability feature (v4.7.0): `tests/unit/test_ai_timeline_text.py`, `tests/integration/test_jd_resolve.py`, `tests/integration/test_ai_scoring.py`, `tests/integration/test_applications_score_all_api.py`, plus extensions to `test_cv_extract.py`, `test_database_migrations.py`, `test_ai_provider_flow.py`, `test_applications_api.py`, and `test_sync_targeted_do_sync.py`. Match-signal audit detail (v4.7.6) added `tests/unit/test_sync_common_match_signal.py` + `tests/component/test_sync_common_contact_link_audit.py`. The Rapport Agent permission monitor + footer-phone date-parsing fix (v4.7.7) added `agent/tests/test_permission_monitor.py` (8 tests) and `tests/unit/test_sync_common_footer_phone.py` (14 tests). The v4.7.8 local_files JD-resolution fix extended `test_jd_resolve.py` with a `TestResolveJdTextsLocalFiles` class (5 tests). v4.7.9 (score-all active-only scope) added one case to `test_applications_score_all_api.py`. v4.7.10 (audit-log scroll-chaining fix) added 2 cases to `AuditLogModal.test.tsx`. v4.7.11 (per-source contacts progress display) added a second `ContactsView.syncProgress.test.tsx` case. A live-scraping LinkedIn-contacts-sync approach was built and shipped in v4.7.11/v4.7.12 with its own test files, then replaced in v4.7.13 by a CSV import (see below) — those scraper-specific test files were removed along with the feature they tested. v4.7.13 (LinkedIn connections CSV import, replacing the live scraper) added `tests/unit/test_linkedin_connections_csv.py` (6 tests) and `tests/api/test_linkedin_connections_import_api.py` (6 tests); the `SyncSettings.linkedin_contacts_enabled` column/migration from v4.7.11 stays in place (SQLite migrations are additive-only in this codebase) even though nothing reads it anymore — its migration test (`TestMigrateSyncSettingsLinkedinContacts` in `test_database_migrations.py`) still accurately tests real, still-existing code. v4.7.14 rebuilt the LinkedIn messages sync back to live scraping (reverting v4.7.7's CSV-import approach, an explicit risk-accepted decision — see §3.4 in ARCHITECTURE.md) and added per-category live progress (status/current_page) to the job sync: `tests/api/test_sync_linkedin_messages_api.py` was fully rewritten (19 CSV-upload-based tests → 14 tests covering `_scrape_linkedin_messages()`/`_upsert_linkedin_messages()` directly with mocked Playwright objects, plus `attach_linkedin_messages_for_contact()`'s matching/date-floor/umlaut behavior — unchanged from the CSV era — driven straight against the DB instead of via file upload); `KanbanBoard.aiScoring.test.tsx` and other existing suites were unaffected. No dedicated test was added for the per-category progress fields (`status`/`current_page` on `LinkedInSyncCategoryCount`) beyond the existing `ruff`/`tsc` type-checking — the underlying `_scrape_category()`/`_cc_entry()` logic was already covered, only new struct fields were threaded through. v4.7.16 (candidate count + AI-assessment finetuning + persistent rapportGPT feedback) added new `tests/unit/test_historical_outcomes.py` (8 cases for `compute_stage_outcomes()`'s same-pipeline-stage comparison) and `tests/unit/test_ai_chat_tools_feedback.py` (6 cases for the new `add_assessment_feedback` chat tool, incl. tenant-scoping and the append-only-log guarantee), plus extensions to `test_database_migrations.py` (`_migrate_bewerberzahl()`), `test_linkedin_job_description.py` (the new `_APPLICANT_COUNT_JS` heuristic, hit/miss/exception cases), `test_applications_events_contacts_api.py` and `test_applications_api.py` (`bewerberzahl` create/update/audit + LinkedIn-extract prefill), `test_ai_provider_flow.py` (stricter match-score system-prompt wording, the new activity-stats/history/feedback prompt blocks), and `test_ai_scoring.py` (`score_application()` end-to-end with all three new signals present). Frontend: `ApplicationModal.aiScoring.test.tsx` extended with 4 new cases (feedback list newest-first, no-feedback-section, candidate-count set/unknown).
...
**Event-loop safety (a rare L3 case worth calling out):** an `async def` function that calls a synchronous, blocking library directly (imaplib, caldav — neither has an async variant) can freeze the *entire app's* single event loop for every request while it runs, not just the sync in progress. This class of bug is invisible to a normal mocked test — the function returns the same result whether the blocking call is offloaded to a thread or not, just at very different cost to the rest of the app while it's running (confirmed live 2026-07-14: a slow iCloud CalDAV response froze the whole app for ~20 minutes; fixed via `asyncio.to_thread()`, see `sync_icloud.py`). The only way to test for this concretely is timing-based concurrency: run the function alongside a sibling coroutine doing `await asyncio.sleep(...)`, and assert the sibling's elapsed time (measured from *before* both start, not from inside the sibling itself — an easy mistake that makes the test pass regardless) stays close to its own sleep duration instead of being delayed by the blocking call. See `tests/integration/test_icloud_calendar_sync.py::TestDoIcloudCalDoesNotBlockEventLoop` for the pattern — verified to actually catch the regression by temporarily reverting the fix and confirming the test failed, not just written and assumed to work.

**Related but distinct case — CPU-bound work that can't be offloaded, only killed:** `pdfplumber`'s PDF text extraction (`app/cv_extract.py`, used by the AI-assessment CV feature) can pathologically spin at ~100% CPU for minutes on certain real-world PDFs without ever raising or returning — confirmed live 2026-07-16, when a migration backfilling `cv_extracted_text` for a pre-existing CV upload hung the whole app's startup this way (a real ~5-minute outage). This isn't fixable with `asyncio.to_thread()` like the I/O-bound case above — that only moves *where* the CPU burn happens, a hung thread just keeps eating CPU forever in the background. Only killing the process stops it, hence `extract_cv_text()` running the actual parse in a `multiprocessing` subprocess bounded to a hard timeout (`app.cv_extract.EXTRACTION_TIMEOUT_SECONDS`), terminated if it overruns. Tested the same way in spirit as the event-loop case — prove the call returns promptly instead of hanging — but simpler to force deterministically: pass a timeout so short (`timeout=0.01`) that even a trivial, fast extraction can't complete a fresh subprocess spawn within it, then assert the call still returns `None` well under the test's own patience window. See `tests/unit/test_cv_extract.py::TestExtractCvText::test_negativ_ueberschreitet_timeout_liefert_none_statt_zu_haengen`.
...
| **AI provider (litellm)** | HTTP to Groq/Anthropic/OpenAI/Gemini | Fake provider implementation that returns deterministic JSON responses (including deliberately broken/empty responses for error-case tests); for L3 integration tests, additional `respx` mocks at the HTTP level to also test `litellm`'s own rate-limit/auth-error handling |
...
2. **Integration tests (L1):** Factory dispatch (`make_files_provider()`/`make_notes_provider()`/`make_calls_provider()` returns the correct type per platform, raises `NotImplementedError` otherwise) covered in `test_providers_factory.py`. `service.is_registered()`/`register()`/`unregister()` dispatch covered in `test_service.py`, both with mocked `subprocess`/`platform.system()`. `_plist_contents()`/`_command_line()`/`_service_content()`'s generated config content (the command/args split each service backend needs) covered in `test_launchd.py`/`test_registry_run.py`/`test_systemd_service.py`.

...
## Work State (Session v4.7.0 – 2026-07-31)

Picks up after v4.6.65 (below). Between v4.6.65 and this session: contacts-sync backfill matching by surname instead of full name (v4.6.66), LinkedIn sync richer live progress details (v4.6.67), iPad UI optimization + Analytics chart render bug fix (v4.6.68), and a CI fix for the actual root cause behind repeated backend-job hangs — an unresolvable Seq DNS lookup on the hosted runner (v4.6.69).

### Completed in This Session

**AI match score & success probability (v4.7.0), the user's explicit feature request:** two new AI-computed fields per application — `match_score` (0-100, how well the job fits the account's CV + LinkedIn profile) and `success_probability` (0-100, combining the match with real timeline activity/ghosting/pipeline stage) — recomputed on every sync, both individual per-application sync and the global "Sync all". Three binding decisions were made explicitly by the user via `AskUserQuestion` before implementation (each is the non-default/non-recommended option): JD source is the *existing* Attachments tab, not a new dedicated upload field (up to 3 most-recent file attachments' extracted text, labeled by filename, fed to the LLM — the model itself judges which one is the actual posting, since there's no way to mark "the" JD); `success_probability` is computed via a **second, separate** AI call (not a deterministic formula), fed the match score + full timeline text + ghosting flag; and both scores are **always recomputed** on every sync, never skipped based on "already scored."

Backend: `ai/jd_resolve.py` (new) resolves JD text from attachments, falling back to a cached LinkedIn-posting scrape (`Application.jd_link_text_cache`, invalidated on `stellenanzeige_url` change) only when no attachment has extractable text — an application with neither stays unscored rather than the model guessing. `cv_extract.py` was generalized (`extract_text_from_file()`) to serve both the original CV-upload path and this new JD-attachment path, reusing the existing subprocess-timeout-wrapped extraction (a documented 2026-07-16 incident showed pdfplumber can hang indefinitely on certain PDFs). `ai/tasks.py` gained `compute_match_score()`/`compute_success_probability()`; `routers/applications.py::score_application()` orchestrates both calls per application (skipping the second for terminal `rejected`/`signed` statuses, setting 0/100 deterministically instead — the outcome is already known) and is wired into `sync_targeted.py`'s `_do_sync()` as a best-effort last step (try/except — a broken AI setup must never fail the sync itself). A new `POST /api/applications/score-all` batch endpoint reuses the same `sync_common.py` progress/batch-result mechanism the other global sync sources use (new `ai_scoring` key), with the same provider-based throttle (5s gemini/groq, 1s else) the removed traffic-light assessment used, stopping the whole batch on `AINotConfigured`/`AIRateLimited` but continuing past any other per-app error.

Frontend: Kanban card badges (color-banded ≥70 green/40-69 yellow/<40 red, tooltip carries the reasoning), two new sortable table columns (with the easy-to-miss group-header `colSpan` bumped accordingly), and a new Overview-tab section with both scores + full reasoning + an explicit "not yet computed" state (distinct from a literal 0) in `ApplicationModal.tsx`. `SyncButton.tsx` fires the new batch endpoint gated on the AI provider actually being configured (`api.settings.getAi()`).

Reused the removed traffic-light AI assessment's prompt-engineering patterns, CV/LinkedIn-profile caching, and post-sync hook point (recovered via `git show` on the commits that added/removed it) rather than reinventing already-solved subproblems — the traffic-light *semantics* themselves were not resurrected, only the reusable plumbing.

All 1840 backend tests (566 unit / 341 component / 676 api / 257 integration) and 167 frontend tests pass, ruff/tsc/production build clean.

### Open / Next Steps
- No E2E journey covers the new AI-scoring flow specifically (sync triggers scoring → Kanban/table/modal show it) — not requested this session, worth considering if it proves central
- Live-browser verification wasn't possible in this dev environment (same `rapport-frontend-dev` Vite-proxy-to-Docker-hostname limitation noted in the v4.5.2 session below) — verified via `tsc`/`vitest`/production build + full backend suites instead

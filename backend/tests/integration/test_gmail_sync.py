...
class TestDoGmailNeueNachrichten:
    ...
    async def test_positiv_absage_erzeugt_statusvorschlag_statt_direkter_aenderung(
        ...
        app = application_factory(db_session, firma="Contoso AG", main_status="applied")
        seed_floor(db_session, app)
    ...
    async def test_positiv_phase2_erkennt_firmenname_der_nur_im_mailbody_steht(
        ...
        app1 = application_factory(db_session, firma="Contoso AG")
        seed_floor(db_session, app1)
        contact = contact_factory(db_session, email="recruiterin@contoso.com")
        app1.contacts.append(contact)
        app2 = application_factory(db_session, firma="Globex Corp")
        db_session.commit()
    ...
    async def test_negativ_mail_ohne_floor_wird_uebersprungen(self, db_session, google_sync, fake_gmail):
        ...
        meta, full = gmail_message(
            "msg-4", "Recruiterin <recruiterin@contoso.com>", "Interview",
        ...
        assert result["errors"] == []
        assert result["created"] == 0
        assert db_session.query(models.Event).filter_by(source="gmail", external_id="msg-4").first() is None

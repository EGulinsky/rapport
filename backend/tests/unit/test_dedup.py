...
class TestNormFirma:
    def test_positiv_einfacher_name(self):
        assert norm_firma("Contoso") == "contoso"

    def test_positiv_gross_klein_ignoriert(self):
        assert norm_firma("CONTOSO") == norm_firma("contoso")

    def test_positiv_rechtsform_wird_entfernt(self):
        # Rechtsformen wie "AG"/"GmbH" dürfen den Match nicht verhindern —
        # sonst würden "Contoso" und "Contoso AG" als unterschiedliche Firmen gelten.
        assert norm_firma("Contoso AG") == norm_firma("Contoso")

    def test_corner_case_mehrfaches_leerzeichen(self):
        assert norm_firma("Contoso   Digital") == norm_firma("Contoso Digital")

    def test_negativ_unterschiedliche_firmen_bleiben_unterschiedlich(self):
        assert norm_firma("Contoso") != norm_firma("Fabrikam")
...
class TestDedupKey:
    def test_positiv_gleiche_bewerbung_gleicher_key(self):
        assert dedup_key("Contoso AG", "Entwickler (m/w/d)") == dedup_key("Contoso", "Entwickler")

    def test_negativ_andere_rolle_anderer_key(self):
        assert dedup_key("Contoso", "Entwickler") != dedup_key("Contoso", "Manager")

    def test_corner_case_gleiche_firma_leere_rolle(self):
        # Muss deterministisch bleiben, nicht crashen.
        k1 = dedup_key("Contoso", "")
        k2 = dedup_key("Contoso", "")
        assert k1 == k2
